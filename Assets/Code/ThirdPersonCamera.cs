...
public sealed class ThirdPersonCamera : MonoBehaviour
{
    ...
    [SerializeField, Min(0)] float _orbitSpeed = .5f;
    [SerializeField] Vector2 _verticalLimits = new Vector2(-30, 80);
    [SerializeField] Vector2 _defaultAngle = new Vector2(30, 0);

    [Header("Physics Settings")]
    [SerializeField] LayerMask _collisionLayer = 0x1;
    [SerializeField] float _collisionResetSpeed = 2;
    ...
    private float GetSafeOrbitDistance(Vector3 targetPosition, Vector3 cameraPosition, Quaternion cameraRotation, float maximumDistance)
    {
        Vector3[] clippingPoints = GetCameraClippingPoints(cameraPosition, cameraRotation,
            _camera.fieldOfView, _camera.nearClipPlane, _camera.aspect);
        ...
        for (int i = 0; i < clippingPoints.Length; i++)
        {
            ...
            if (Physics.Raycast(targetPosition, direction, out RaycastHit hit, maximumDistance, _collisionLayer))
            {

                float distance = hit.distance * Mathf.Cos(_camera.fieldOfView * .5f * Mathf.Deg2Rad);
    ...
    private Vector3[] GetCameraClippingPoints(Vector3 position, Quaternion rotation, float fov, float nearClip, float aspect)
    {
